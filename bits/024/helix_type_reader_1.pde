...
// Original text by Soren Kierkegaard ("The Rotation Method"...)
...
void setup()
{
  ...
  f = loadFont("Century-Bold.vlw.gz");  // don't forget to have this font included in your "data" folder...
  Text txt = new Text(lines[0], f, colors[0], 10.0);

  helix = new Helix(0.0, 0.0, -30.0, 3.0, 80.0, 25.0, 100.0, txt);
  helix.setOffset(Helix.END);
  helix.setSlidingSpeed(0.0f);
  helix.setSlidingAcceleration(-1.0f);
  helix.startSliding();

  hscrollbar = new ScrollBar(8.0, 280.0, 283.0, 13.0, ScrollBar.LEFT, ScrollBar.TOP, ScrollBar.HORIZONTAL, 0.0, 2.0 * helix.getLength() + txt.getWidth(), 0.0, helix.getLength(), color(153, 153, 153), color(204, 204, 204), true);
  hscrollbar.setUnitIncrement(10.0 / framerate() * 2.0);  // while a scrollbar button is pressed, the text is scrolling at (2 * fontSize) pixels per second...
}

void loop()
{
  hscrollbar.run();  // keep it at the beginning...
  ...
  if (!helix.sliding)
  {
    helix.offset = -hscrollbar.getValue() + helix.getLength();
  }
  ...
  if (helix.sliding)
  {
    hscrollbar.setValue(helix.getLength() - helix.offset);
  }

  hscrollbar.draw();  // keep it at the end...
}
